...
if submitted:
    if "user_id" not in st.session_state:
        st.session_state["user_id"] = str(uuid.uuid4())  # generate if not already set

    survey_data = {
        "id": str(uuid.uuid4()),
        "user_id": st.session_state["user_id"],   # ✅ add user ID
...
with col2:
    if st.button("🤖 Talk to AI Assistant"):
        st.switch_page("pages/9_chatbot_assistant.py")
